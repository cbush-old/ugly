...
class BasicFramebuffer {
  ...
  public:
    ...
    virtual void draw(ProgramConstRef, VertexArray const&, GLenum mode, size_t count, size_t first = 0) =0;
  
    /**
     * @brief draw using the given VertexArray object's 'mode' and 'count'.
     **/
    virtual void draw(ProgramConstRef, VertexArray const&);
  ...
  , public BasicFramebuffer
{
  ...
  public:
    ...
    void draw(ProgramConstRef program, VertexArray const&, GLenum mode, size_t count, size_t first = 0) override;
